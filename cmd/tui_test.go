...
func TestListHeaderLines_WithBranch(t *testing.T) {
	...
	// blank + brand box (13 lines, tabs in bottom border) = 14, regardless of git branch
	if got := m.listHeaderLines(); got != 14 {
		t.Errorf("expected 14 header lines with git branch, got %d", got)
	}
}
...
func TestListHeaderLines_WithoutBranch(t *testing.T) {
	m := &model{}
	if got := m.listHeaderLines(); got != 14 {
		t.Errorf("expected 14 header lines without git branch, got %d", got)
	}
}
...
func TestVisibleHeight_Normal(t *testing.T) {
	m := &model{height: 24}
	// overhead = 14 (header) + 6 = 20; visible = 24 - 20 = 4
...
func TestVisibleHeight_WithBranch(t *testing.T) {
	m := &model{height: 24, gitBranch: "main"}
	// overhead = 14 (header) + 6 = 20; visible = 24 - 20 = 4
	if got := m.visibleHeight(); got != 4 {
