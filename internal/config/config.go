...
// Config is the top-level structure parsed from knot.yml.
type Config struct {
...
// Load reads and parses a knot.yml file at the given path.
...
// FindConfigFile walks upward from startDir looking for a knot.yml file.
// Returns the absolute path to the first knot.yml found, or an error if none exists.
func FindConfigFile(startDir string) (string, error) {
	...
	for {
		candidate := filepath.Join(dir, "knot.yml")
		...
		if parent == dir {
			...
			return "", fmt.Errorf("knot.yml not found (searched from %q upward)", startDir)
		}
		dir = parent
	}
}
